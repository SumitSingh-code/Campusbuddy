...
-- need SELECT access through RLS on messages and notifications.
...
-- UPDATE: only service_role (via API) updates last_message_id, unread_counts
-- Participants CAN mark-read (zero out their own unread count):
CREATE POLICY "conversations_update_self" ON public.conversations FOR UPDATE TO authenticated
  USING (participant_1 = auth.uid() OR participant_2 = auth.uid())
